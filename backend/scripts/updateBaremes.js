...
async function updateBaremes() {
    try {
        ...
        // 4. Questions sur le Coran
        await db.query(`
            UPDATE rubriques 
            SET 
                points_max = 10,
                temps_par_question = 15,
                criteres_evaluation = '{"exactitude": 5}'::jsonb,
                description = '2 questions - 5 pts par bonne réponse - 15 sec/question'
            WHERE nom = 'Questions sur le Coran'
        `);
        console.log('✅ Questions sur le Coran: 10 pts (5 pts/question) - 2 questions de 15 sec');
